unit MainFm;
//---------------------------------------------------------------------------
// Basic GUI example for Asphyre.                       Modified: 26-May-2013
// Illustrates how to use and work with Asphyre GUI framework.   Version 1.02
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
uses
  Types, SysUtils, Classes, Controls, Forms, Dialogs;
...
begin
 ...
 vFont.Kernings.Spec('F', 'r', -1);
 vFont.Kernings.Spec('R', 'a', -2);
 vFont.Kernings.Spec('a', 'm', -1);
 vFont.Kernings.Spec('a', 't', -2);
 vFont.Kernings.Spec('r', 'a', -1);
 vFont.Kernings.Spec('t', 'e', -1);
end;
 ...
 if (SameText(EventControlName, 'AddButton')) then
  begin
   if (GetControlChecked(GameTaskbar, 'AnonymBox')) then
    vText:= 'A' + IntToStr(AddItemNo + 1)
     else vText:= 'N' + IntToStr(AddItemNo + 1);

   vText:= vText + ': ' + GetControlText(GameTaskbar, 'UserNameEdit');

   Index:= ListBox.Items.Add(vText);
